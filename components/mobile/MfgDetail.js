...
function MfgDetailPage({mfgRecords,searchQuery}) {
    ...
    useEffect(() => {
      const filterFieldsOrder = (item) => {
        const {
            id, module, drawing_no, location, EOL, supplier, client, client_name, regrind_count, ...filterFieldsMfg 
        } = item;
        return filterFieldsMfg;
      };
  
      const filteredRecordOrder = mfgRecords.map(filterFieldsOrder);
